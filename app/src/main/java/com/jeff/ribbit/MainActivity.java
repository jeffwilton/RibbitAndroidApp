...
public class MainActivity extends FragmentActivity implements ActionBar.TabListener {
            ...
            if(isExternalStorageAvailable()) {
                ...
                if(!mediaStorageDir.exists()) {
                    if(!mediaStorageDir.mkdirs()) {
                        Log.e(TAG, getString(R.string.error_create_dirs));
                        return null;
                    }
                }

                //3. create a filename
                //4. create an actual file
                File mediaFile;
                Date now = new Date();
        ...
        if(resultCode == RESULT_OK) {
            ...
            if(resultCode == PICK_PHOTO_REQUEST || resultCode == PICK_VIDEO_REQUEST) {
                ...
                if(requestCode == PICK_VIDEO_REQUEST) {
                    // make sure the filesize is less than 10 MB
                    int filesSize = 0;
                    InputStream inputStream = null;
                    try {
                        inputStream = getContentResolver().openInputStream(mMediaUri);
                        filesSize = inputStream.available();
                    }
                        ...
                        catch (IOException ioExc) { /* intentionally blank */ }
                    }
    ...
    private void navigateToLogin() {
        Intent intent = new Intent(this, LoginActivity.class); //all activities are started through intents
